...
class FileListWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        self.loading_label.setProperty(
            "loading", "true"
        ...
        self.empty_label.setStyleSheet(
            "color: #8B949E; font-size: 12px; padding: 20px;"
        )
    ...
    def eventFilter(self, watched, event):
        ...
        if watched == self.table:
            if event.type() == QEvent.KeyPress:
                logger.debug(
                    f"Key pressed in table: key={event.key()}, modifiers={event.modifiers()}"
                )
        ...
        if self.load_thread:
            try:
                if self.load_thread.isRunning():
                    ...
                    if self.load_thread.isRunning():
                        logger.warning(
                            "Load thread did not finish in 2 seconds, forcing termination"
                        )
    ...
    def load_directory(self, path: str, show_hidden: bool = False):
            ...
            try:
                if self.load_thread.isRunning():
                    ...
                    if not self.load_thread.wait(500):
                        logger.warning(
                            "Previous load thread did not finish in 2 seconds, forcing termination"
                        )
                        self.load_thread.terminate()
                        self.load_thread.wait()
            except RuntimeError as e:
        ...
        self._show_loading()
        logger.info(
            f"Loading directory asynchronously: {path} (show_hidden={show_hidden})"
        )

        self.load_thread = DirectoryLoadThread(
            self.file_ops, path, show_hidden=show_hidden
        )
        ...
        if self.loading_label.isVisible() and self.content_container:
            ...
            if parent_size.width() > 0 and parent_size.height() > 0:
                self.loading_label.setGeometry(
                    0, 0, parent_size.width(), parent_size.height()
                )
    ...
    def _on_load_error(self, error_msg: str):
        ...
        self.empty_label.setText(
            language_manager.tr("file_list.error", message=error_msg)
        )
        self.empty_label.show()
        self.table.hide()
        self.selection_changed.emit()
    ...
    def _display_files(self):
        ...
        for row, file_info in enumerate(self.files):
            ...
            if file_info.is_dir:
                ...
                name_item.setIcon(
                    icon_manager.get_file_type_icon(file_info.name, False)
                )
            ...
            if file_info.is_dir:
                size_item = QTableWidgetItem(
                    language_manager.tr("file_list.dir_placeholder")
                )
