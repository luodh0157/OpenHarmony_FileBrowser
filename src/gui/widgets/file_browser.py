...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QSplitter, QToolBar, QFileDialog, QDialog, QStyle, QMessageBox
...
class FileBrowserWidget(QWidget):
    ...
    def setup_toolbar(self, toolbar: QToolBar):
        ...
        refresh_action = QAction(icon_manager.get_icon('refresh'), language_manager.tr('toolbar.refresh'), self)
        ...
        new_folder_action = QAction(icon_manager.get_icon('new_folder'), language_manager.tr('toolbar.new_folder'), self)
        ...
        rename_action = QAction(icon_manager.get_icon('rename'), language_manager.tr('toolbar.rename'), self)
        ...
        delete_action = QAction(icon_manager.get_icon('delete'), language_manager.tr('toolbar.delete'), self)
        ...
        upload_action = QAction(icon_manager.get_icon('upload'), language_manager.tr('toolbar.upload'), self)
        ...
        download_action = QAction(icon_manager.get_icon('download'), language_manager.tr('toolbar.download'), self)
    ...
    def _on_file_double_clicked(self, path: str):
        ...
        if not self.preview_handler.can_preview(selected_file.name, selected_file.size):
            show_error_dialog(
                ...
                language_manager.tr('dialogs.cannot_preview_msg', name=selected_file.name),
                self
            )
            return
    ...
    def _create_folder(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if not folder_name:
                show_error_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('dialogs.folder_name_empty'), self)
                return
    ...
    def _rename(self):
        ...
        if not selected_file:
            show_error_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('dialogs.no_files_selected'), self)
            return
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if not new_name:
                show_error_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('dialogs.name_empty'), self)
                return
    ...
    def _delete(self):
        ...
        if count <= 5:
            ...
            message += language_manager.tr('dialogs.delete_items_list') + "\n" + "\n".join(names)
        else:
        ...
        btn_yes = msg_box.addButton(language_manager.tr('buttons.yes'), QMessageBox.YesRole)
        btn_no = msg_box.addButton(language_manager.tr('buttons.no'), QMessageBox.NoRole)
    ...
    def _upload_file(self):
        ...
        btn_files = msg.addButton("📁 " + language_manager.tr('dialogs.select_files'), QMessageBox.ActionRole)
        btn_folder = msg.addButton("📂 " + language_manager.tr('dialogs.select_folder'), QMessageBox.ActionRole)
