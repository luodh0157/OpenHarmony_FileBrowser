...
class FileTreeWidget(QWidget):
    ...
    def _populate_directories(self, parent_item: QTreeWidgetItem, directories: List[FileInfo]):
        ...
        logger.debug(f"Populated {len(directories)} directories under: {parent_item.text(0)}")
    
    def _load_directory_contents_async(self, path: str, parent_item: QTreeWidgetItem, delay_ms: int = 0):
        """
        ...
        if delay_ms > 0:
            QTimer.singleShot(delay_ms, lambda: self._do_load_directory(path, parent_item))
        else:
    ...
    def _do_load_directory(self, path: str, parent_item: QTreeWidgetItem):
        ...
        thread.error.connect(
            lambda err, p=path: self._on_tree_error(p, err)
        )
        
        self._load_threads[path] = thread
        thread.start()
    
    def _on_tree_loaded(self, path: str, parent_item: QTreeWidgetItem, directories: List[FileInfo]):
    ...
    def expand_to_path(self, path: str):
        ...
        for part in parts:
            ...
            if first_child and first_child.text(0) == loading_text:
                ...
                self._load_and_expand(current_item, target_path, remaining, delay_ms=150)
                return
    ...
    def _load_and_expand(self, parent_item: QTreeWidgetItem, target_path: str, remaining_parts: List[str], delay_ms: int = 150):
        """
        ...
        def on_loaded(directories):
            ...
            for i in range(parent_item.childCount()):
                ...
                if child.text(0) == next_part:
                    child.setExpanded(True)
                    self._expand_remaining(child, current_path, next_part, remaining_parts[1:])
                    break
        ...
        cached_dirs = self.file_ops.cache.get_child_dirs(target_path.rsplit("/", 1)[0] or "/")
        if cached_dirs:
            on_loaded(cached_dirs)
            return
        
        if delay_ms > 0:
            QTimer.singleShot(delay_ms, lambda: self._start_tree_load(target_path.rsplit("/", 1)[0] or "/", on_loaded, on_error))
        else:
            self._start_tree_load(target_path.rsplit("/", 1)[0] or "/", on_loaded, on_error)
    ...
    def _start_tree_load(self, path, on_loaded, on_error):
        ...
        thread = TreeLoadThread(self.file_ops, path)
        thread.loaded.connect(lambda dirs: self._on_tree_thread_loaded(path, dirs, on_loaded))
        thread.error.connect(lambda err: self._on_tree_thread_error(path, err, on_error))
    ...
    def _expand_remaining(self, current_item, current_path, part, remaining_parts):
        ...
        if first_child and first_child.text(0) == loading_text:
            ...
            self._load_and_expand(current_item, target_path, remaining_parts, delay_ms=150)
            return
