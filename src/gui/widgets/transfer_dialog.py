...
class TransferDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.status_label = QLabel(language_manager.tr('transfer.status', completed=0, total=0))
        ...
        self.total_size_label = QLabel(language_manager.tr('transfer.total', size="0 bytes"))
        summary_layout.addWidget(self.total_size_label)
        
        summary_layout.addStretch()
        
        self.avg_speed_label = QLabel(language_manager.tr('transfer.speed', speed="0 B"))
    ...
    def update_language(self):
        ...
        # Update status label
        self.status_label.setText(language_manager.tr('transfer.status', completed=self.completed_count, total=self.total_count))
    ...
    def _update_table_language(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if direction_item:
                path = self.table.item(row, 0).data(Qt.UserRole) if self.table.item(row, 0) else None
                if path and path in self.tasks:
                    task = self.tasks[path]
                    direction_text = language_manager.tr('transfer.upload') if task.direction == TransferDirection.UPLOAD else language_manager.tr('transfer.download')
    ...
    def _set_table_row(self, row: int, task: TransferTask):
        ...
        size_text = format_file_size(task.total_size) if task.total_size > 0 else "Unknown"
        size_item = QTableWidgetItem(size_text)
        self.table.setItem(row, 3, size_item)
        
        speed_text = f"{format_file_size(int(task.speed))}/s" if task.speed > 0 else "0 B/s"
    ...
    def _update_summary(self):
        ...
        self.total_size_label.setText(language_manager.tr('transfer.total', size=format_file_size(total_size)))
        ...
        self.avg_speed_label.setText(language_manager.tr('transfer.speed', speed=format_file_size(int(avg_speed))))
