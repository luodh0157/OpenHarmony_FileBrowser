...
class PreviewWindow(QDialog):
    ...
    def update_language(self):
        ...
        self.setWindowTitle(language_manager.tr('preview.title'))
        if self.file_name:
            self.setWindowTitle(language_manager.tr('preview.preview_title', name=self.file_name))
        
        self.file_label.setText(self.file_name if self.file_name else language_manager.tr('preview.no_file'))
        if self.file_size > 0:
            self.size_label.setText(language_manager.tr('preview.bytes', size=self.file_size))
    ...
    def preview_file(self, remote_path: str, file_name: str, file_size: int):
        ...
        if not self.preview_handler:
            show_warning_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('preview.handler_not_init'), self)
            return
        ...
        self.setWindowTitle(language_manager.tr('preview.preview_title', name=file_name))
        ...
        if not self.local_file_path:
            show_warning_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('preview.download_failed'), self)
            return
    ...
    def _preview_image(self):
        ...
        try:
            ...
            if not image:
                show_warning_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('preview.open_image_failed'), self)
                return
            ...
            show_warning_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('preview.preview_image_error', error=str(e)), self)
    ...
    def _preview_video(self):
        ...
        self.video_info_label.setText(
            language_manager.tr('preview.video_info', name=self.file_name, size=self.file_size)
        )
    ...
    def _play_video(self):
        ...
        success = self.preview_handler.open_video_with_system_player(self.local_file_path)
        
        if success:
            logger.info(f"Video opened with system player")
        else:
            show_warning_dialog(language_manager.tr('dialogs.error_title'), language_manager.tr('preview.open_video_failed'), self)
