...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(
            f"{language_manager.tr('app_name')} {config.app_version}  ({language_manager.tr('author')})"
        )
    ...
    def _init_ui(self):
        ...
        refresh_devices_btn = QAction(
            icon_manager.get_icon("refresh_devices"),
            language_manager.tr("toolbar.refresh_devices"),
            self,
        )
    ...
    def _init_theme_manager(self):
        ...
        logger.info(
            f"Theme manager initialized with theme: {self.theme_manager.current_theme}"
        )
    ...
    def _update_menu_language(self):
        ...
        if self.language_action:
            ...
            self.language_action.setToolTip(
                language_manager.tr("menu.language_tooltip")
            )
    ...
    def _create_menu_bar(self):
        ...
        self.theme_action = QAction(
            icon_manager.get_icon("sun"), language_manager.tr("menu.theme"), self
        )
        self.theme_action.setToolTip(language_manager.tr("menu.theme_tooltip"))
    ...
    def _auto_refresh_devices(self):
        ...
        try:
            ...
            if len(devices) > 0:
                ...
                self.file_count_label.setText(
                    language_manager.tr(
                        "status.found_devices_connecting", count=len(devices)
                    )
                )
                ...
                self.file_count_label.setText(
                    language_manager.tr("status.no_devices_found")
                )
            ...
            self.file_count_label.setText(
                language_manager.tr("status.scan_error", e=str(e))
            )
    ...
    def _refresh_devices(self):
        ...
        if self.device_manager:
            ...
            if len(devices) > 0:
                self.file_count_label.setText(
                    language_manager.tr("status.found_devices", count=len(devices))
                )
            else:
                self.file_count_label.setText(
                    language_manager.tr("status.no_devices_found_short")
                )
