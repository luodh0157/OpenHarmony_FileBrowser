...
class TransferTask:
    ...
    original_mtime: Optional[float] = None  # Original modification time (Unix timestamp)
        ...
        for item in dir_path.rglob('*'):
            if item.is_file():
                try:
                    ...
                    remote_item_path = f"{self.task.remote_path.rstrip('/')}/{relative_path}"
                    
                    try:
                        stat_info = self.hdc.shell_stat(self.task.device_id, remote_item_path)
                        if stat_info.modified_time:
                        ...
                        if self.task.original_mtime:
                            os.utime(str(item), (self.task.original_mtime, self.task.original_mtime))
                except Exception as e:
...
class TransferManager(QObject):
    ...
    def add_upload_task(self, device_id: str, local_path: str, remote_path: str, 
        ...
        logger.info(f"Upload task added: {local_path} -> {remote_path} (preserve_ts={preserve_timestamp})")
    
    def add_download_task(self, device_id: str, remote_path: str, local_path: str,
                          preserve_timestamp: bool = True):
        """
        Add download task.
        ...
        logger.info(f"Download task added: {remote_path} -> {local_path} (preserve_ts={preserve_timestamp})")
    ...
    def clear_completed_tasks(self):
        ...
        completed = [t for t in self.tasks if t.status in [
